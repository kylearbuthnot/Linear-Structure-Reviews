...
int evalRPN(vector<string>& tokens) {
    ...
    for (int i = 0; i < tokens.size(); i++) {
        ...
        if (isValidNumber(tokens[i])) {//if we are looking at a number, push it onto the integer stack and convert it to int.
            ...
            if (integers.size() >= 2) {
                int intRight = integers.top();
                integers.pop();
                int intLeft = integers.top();
                integers.pop();
                ...
                case '+': //if its a plus, add the two numbers and push the result onto the integer stack.
					integers.push(intLeft + intRight);
